...
NeuraxConfig.ScanHostInterval    | Time interval to sleep before scanning next host in active mode | `"none"` 
NeuraxConfig.ScanGatewayFirst | Gateway is the first host scanned when active scan is used | `false`
NeuraxConfig.ScanOnly | NeuraxScan() scans only those hosts | `[]string{}`
...
NeuraxConfig.WordlistCommonCountries| A map[string]int that contains country codes and number of passwords to use| map[string]int
NeuraxConfig.WordlistMutators | Mutators to use when `.WordlistExpand` is specified | `{"single_upper", "cyryllic", "encapsule"}`
NeuraxConfig.WordlistPermuteNum | Maximum length of permutation generated by NeuraxWordlistPermute()| `2`
NeuraxConfig.WordlistPermuteSeparator | A separator character to use for permutations | `"-"`
